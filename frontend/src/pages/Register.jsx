  ...
  return (
    <div className={styles.root}>
      <aside className={styles.panel}>
        <a href="/" className={styles.panelLogo}>DigiVirasat<span className={styles.dot}>.</span></a>
        <div className={styles.panelBody}>
          <blockquote className={styles.panelQuote}>
            "Ten minutes today.<br />Clarity for a lifetime."
          </blockquote>
          <p className={styles.panelSub}>Start organising your digital life. Your vault is private, secure, and always yours.</p>
        </div>
        <div className={styles.panelSteps}>
          {['Create your vault', 'Add assets & documents', 'Assign nominees'].map((s, i) => (
      ...
      <main className={styles.formSide}>
        <div className={styles.formWrap}>
          <div className={styles.formHeader}>
            ...
            <p className={styles.formSub}>Free to start. No credit card required.</p>
          </div>

          <form className={styles.form} onSubmit={handleSubmit}>
            <div className={styles.field}>
              <label className={styles.label} htmlFor="name">Full name</label>
              <input id="name" type="text" className={styles.input} placeholder="Rahul Sharma" value={name} onChange={(e) => setName(e.target.value)} required autoComplete="name" />
            </div>
            ...
            <div className={styles.field}>
              ...
              <div className={styles.inputWrap}>
                <input id="password" type={showPassword ? 'text' : 'password'} className={styles.input} placeholder="Min. 8 characters" value={password} onChange={(e) => setPassword(e.target.value)} minLength={8} required autoComplete="new-password" />
              ...
              {password && (
                <div className={styles.strengthRow}>
                  <div className={styles.strengthBars}>{[1, 2, 3].map((n) => <div key={n} className={styles.strengthBar} style={{ background: strength >= n ? strengthColor : 'var(--color-border)' }} />)}</div>
            ...
            <button type="submit" className={styles.submitBtn} disabled={submitting}>{submitting ? <span className={styles.spinner} /> : 'Create Vault'}</button>

            <p className={styles.terms}>By creating an account, you agree to our <a href="#">Terms of Service</a> and <a href="#">Privacy Policy</a>.</p>
          </form>
